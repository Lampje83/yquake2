...
int RI_PrepareForWindow(void)
{
	...
	if (gl_debugcontext && gl_debugcontext->value)
	{
...
int RI_InitContext(void* win)
{
	...
	{
		R_Printf(PRINT_ALL, __FUNCTION__"(): loading OpenGL function pointers failed!\n");
		return false;
	}
	...
	snprintf(title, sizeof(title), "Yamagi Quake II %s - OpenGL 4.3", YQ2VERSION);
#if SDL_VERSION_ATLEAST(2, 0, 0)
