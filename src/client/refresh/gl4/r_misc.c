...
R_ScreenShot(void)
{
	...
	// so swap bottom rows with top rows
	{
		...
		byte *rowBuffer = malloc(bytesPerRow);
...
//	R_TexEnv(GL_REPLACE);
	...
	if (gl_config.pointparameters)
	{
		...
		glad_glPointParameterf(GL_POINT_SIZE_MIN_ARB, gl_particle_min_size->value);
		glad_glPointParameterf(GL_POINT_SIZE_MAX_ARB, gl_particle_max_size->value);
		glad_glPointParameterfv(GL_POINT_DISTANCE_ATTENUATION_ARB, attenuations);
	}
/*
