		...
		const unsigned colortable[768])
{
	...
	GLfloat *vtx = malloc(sizeof(GLfloat) * 3 * num_particles * 3);
	GLfloat *tex = malloc(sizeof(GLfloat) * 2 * num_particles * 3);
	GLfloat *clr = malloc(sizeof(GLfloat) * 4 * num_particles * 3);
...
R_DrawParticles(void)
{
	...
	if (gl_config.pointparameters && !(stereo_split_tb || stereo_split_lr))
	{
		...
		GLfloat *vtx = malloc(sizeof(GLfloat) * 3 * r_newrefdef.num_particles);
		GLfloat *clr = malloc(sizeof(GLfloat) * 4 * r_newrefdef.num_particles);
		unsigned int index_vtx = 0;
		unsigned int index_clr = 0;
...
R_Register(void)
{
	...
	gl_debugcontext = ri.Cvar_Get("gl3_debugcontext", "0", 0);
...
qboolean R_InitBuffers() 	{
	// Setup 2D shader
	glGenVertexArrays( 1, &gl_state.program2D.VAO );
	glBindVertexArray( gl_state.program2D.VAO );
	...
	glVertexAttribFormat( ATTRIB_COLOR, 4, GL_FLOAT, GL_FALSE, 12);
	glVertexAttribFormat( ATTRIB_TEXCOORD, 2, GL_FLOAT, GL_FALSE, 28 );
	//glVertexBindingDivisor( gl_state.program2D.VAO, 36 );
	glVertexAttribBinding( ATTRIB_POSITION, 0 );
	glVertexAttribBinding( ATTRIB_COLOR, 0 );
	glVertexAttribBinding( ATTRIB_TEXCOORD, 0 );

	hmm_mat4	defaultMatrix = HMM_Orthographic( 0, 960, 540, 0, -99999, 99999 );

	glBufferData( GL_UNIFORM_BUFFER, sizeof( hmm_mat4 ), &defaultMatrix, GL_DYNAMIC_DRAW );
...
*/
	...
	if (gl_config.major_version < 4)
	{
		{
			R_Printf(PRINT_ALL, "Support for OpenGL 4.x is not available\n");
		}
		return false;
	}
...
*/
	...
	{
		if (glad_glPointParameterf && glad_glPointParameterfv)
		{
