...
GL3_LM_BuildPolygonFromSurface(msurface_t *fa)
{
	...
	/* draw texture */
#if 0 // ERIK: contiguous vertex buffer
	poly = Hunk_Alloc(sizeof(glpoly_t) +
		   (lnumverts - 4) * sizeof(gl3_3D_vtx_t));
#else
	poly = Hunk_Alloc ( sizeof ( glpoly_t ) );
	poly->vertices = &currentmodel->glverts[currentmodel->numglverts];
	currentmodel->numglverts += lnumverts;

	if ( currentmodel->numglverts > currentmodel->maxglverts ) {
		ri.Sys_Error ( ERR_DROP, __FUNCTION__": Too many glverts: %d verts, %d surfedges\n",
			currentmodel->numglverts, currentmodel->numsurfedges );
	}
#endif
	poly->next = fa->polys;
	poly->flags = fa->flags;
