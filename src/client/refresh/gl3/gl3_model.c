...
/*
	...
	for ( i = 0; i < mod->numglverts; i++ ) {
		for ( int j = i + 1; j < mod->numglverts; j++ ) {
			if ( !memcmp ( &mod->glverts[ i ], &mod->glverts[ j ], sizeof ( gl3_3D_vtx_t ) ) ) {
				duplicateverts++;
			}
		}
	}
	R_Printf ( PRINT_DEVELOPER, "%d duplicate vertices found\n", duplicateverts );
*/
	R_Printf ( PRINT_DEVELOPER, "%s:\n", mod->name );
	R_Printf ( PRINT_DEVELOPER, "%d vertices\n", mod->numvertexes );
	R_Printf ( PRINT_DEVELOPER, "%d edges\n", mod->numedges );
	R_Printf ( PRINT_DEVELOPER, "%d surfedges\n", mod->numsurfedges );
	R_Printf ( PRINT_DEVELOPER, "%d planes\n", mod->numplanes );
	R_Printf ( PRINT_DEVELOPER, "%d texinfos\n", mod->numtexinfo );
	R_Printf ( PRINT_DEVELOPER, "%d faces\n", mod->numsurfaces );
	R_Printf ( PRINT_DEVELOPER, "%d marksurfaces\n", mod->nummarksurfaces );
	R_Printf ( PRINT_DEVELOPER, "%d modelsurfaces\n", mod->nummodelsurfaces );
	R_Printf ( PRINT_DEVELOPER, "%d leafs\n", mod->numleafs );
	R_Printf ( PRINT_DEVELOPER, "%d nodes\n", mod->numnodes );
	R_Printf ( PRINT_DEVELOPER, "%d submodels\n", mod->numsubmodels );
