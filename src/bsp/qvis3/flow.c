...
==============
*/
winding_t	*ChopWinding2 (winding_t *in, pstack_t *stack, plane_t *split)
{
	vec_t	dists[128];
	int		sides[128];
	int		counts[3];
...
// check all combinations	
	...
	// source on the back side
		...
		//
			target = ChopWinding2 (target, stack, &plane);
			if (!target)
...
float d;
	...
	{
		stack.pass = ChopWinding2 (p->winding, &stack, &thread->pstack_head.portalplane);
		if (!stack.pass)
			continue;
	}
}
...
float d;
	...
	{
		stack.source = ChopWinding2 (prevstack->source, &stack, &backplane);
		if (!stack.source)
			continue;
	}
}
